...
int create_user_table_entry(sqlite3 *db, std::string jstr, std::string server)
{
  ...
  std::string sqlstatement =
    ...
    + quotesql( std::to_string(root["congestionDetection"].asBool())  ) + ","
    + quotesqlint( std::to_string(root["adsFrequency"].asInt())  ) + ","
    + quotesql( root["dailDownloadQuota"].asString() ) + ","
    + quotesqlint( std::to_string(root["dailyManifestCount"].asInt())  ) + ","
    + quotesqlint( std::to_string(root["dailyDownloadWifi"].asInt())  ) +   ","
    + quotesqlint( std::to_string(root["dailyDownloadCellular"].asInt())  ) +   ","
    + quotesql((get_sdk_capabilities(new_obj)).c_str()) + ","
    + quotesqlint( std::to_string(root["maxContentDuration"].asInt()) ) + ","
